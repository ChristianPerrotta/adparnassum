...
export default function IntervalCanvas({clefs, intervals, notes, index}) {
    ...
    useEffect(() => {
        ...
        if (clefs.length === 1) {
            ...
            notes.forEach((note, i) => {
                ...
                let notesLowerVoice = [new StaveNote({keys: [note.notation], duration: "w", clef: clefs[0]})]
                
                let addedNote = Note.createNoteFromInterval(note, intervals[i])
                let notesUpperVoice = [new StaveNote({keys: [addedNote.notation], duration: "w", clef: clefs[0]})]
                if (i === index) {
        ...
        // each note in separate clef
            ...
            notes.forEach((note, i) => {
                ...
                let addedNote = Note.createNoteFromInterval(note, intervals[i])
                let notesMeasureUpper = [new StaveNote({keys: [addedNote.notation], duration: "w", clef: clefs[0]})];
                ...
                let notesMeasureLower = [new StaveNote({keys: [note.notation], duration: "w", clef: clefs[1]})]
